...
function traverseToParent(target) {
  while (1) {
    if (
      ...
      target.classList.contains("products__item")
    )
      return target;
...
btnAddToCartList.forEach((btn) => {
  ...
  btn.addEventListener("click", () => {
    Swal.fire({
      ...
      confirmButtonText:
        "<a href='/cart.html' class='text-light'>Tới giỏ hàng của bạn</a>",
...
function updateCartView() {
  ...
  if (!getAmount(cart)) {
    content =
      '<span class="text-muted small p-0">Giỏ hàng hiện đang trống</span>';
  } else {
    content += [...cart]
      .map((item) => {
        const name = item[0];
    ...
    content += `
                <div class="cart-dropdown__menu__item">
                    ...
                    <h4 class="product__price ml-auto">${formatCurrency(
                      getCost(cart)
                    )}</h4>
                </div>    
...
function getCost(cart) {
  return [...cart.values()].reduce(
    (totalCost, { price, quantity }) => totalCost + price * quantity,
    0
  );
}
